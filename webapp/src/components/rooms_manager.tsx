...
export function UserItem(user, index) {
    const {room_name} = useParams();

    return (
        <li key={index}
             className={`p-4 hover:bg-gray-600 transition ${user.socketId === room_name ? 'bg-gray-500' : ''}`}>
...
export function RoomsManager(props: { rooms: string[], users: IConnectedUser[], joinRoom: (room: string) => void }) {
...
export function RoomItem(room, index) {
    ...
    return (
        <li key={index}
             className={`p-4 hover:bg-gray-600 transition ${room === room_name ? 'bg-gray-500' : ''}`}>
